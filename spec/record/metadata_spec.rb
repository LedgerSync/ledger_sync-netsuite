...
RSpec.describe LedgerSync::NetSuite::Record::Metadata do
  ...
  before do
    allow_any_instance_of(LedgerSync::NetSuite::Token).to receive(:signature) { 'SIGNATURE' }
    allow_any_instance_of(LedgerSync::NetSuite::Token).to receive(:nonce) { 'NONCE' }
    allow_any_instance_of(LedgerSync::NetSuite::Token).to receive(:timestamp) { '1234567890' }
  end
  ...
  describe '#create' do
    ...
    it { expect(subject).to be_a(LedgerSync::NetSuite::Record::HTTPMethod) }
